...
original_author="alexisolivo"
original_name="energy_monitor_dashboard"
original_urlname="Energy-Monitor-Dashboard"
original_description="Awesome energy_monitor_dashboard created by alexisolivo" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/energy_monitor_dashboard "${name}"
...
then
    ...
    echo "then run 'energy_monitor_dashboard' or 'python -m energy_monitor_dashboard'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'energy_monitor_dashboard' or 'python -m energy_monitor_dashboard' or 'poetry run energy_monitor_dashboard'"
fi
