...
- 🎯 Entry points to execute your program using `python -m <energy_monitor_dashboard>` or `$ energy_monitor_dashboard` with basic CLI argument parsing.
...
---
# energy_monitor_dashboard Flask Application

Awesome energy_monitor_dashboard created by alexisolivo
...
```bash
git clone https://github.com/alexisolivo/Energy-Monitor-Dashboard energy_monitor_dashboard
...
```bash
pip install energy_monitor_dashboard
```
...
```bash
$ python -m energy_monitor_dashboard
```
...
```bash
energy_monitor_dashboard create-db   # run once
energy_monitor_dashboard populate-db  # run once (optional)
energy_monitor_dashboard add-user -u admin -p 1234  # ads a user
energy_monitor_dashboard run
```
